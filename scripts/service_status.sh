...
if [[ "${os_name}" == "Darwin" ]]; then
  exec launchctl print "gui/$(id -u)/${launchd_label}"
fi

if [[ "${os_name}" == "Linux" ]] && command -v systemctl >/dev/null 2>&1; then
  exec systemctl --user status "${systemd_unit}"
fi

echo "service status is only supported on macOS launchd or Linux systemd --user"
exit 1
